...
function App() {
  ...
  // This state stores the unique sample ID generated by /run_model.
  ...
  const handleRunModel = async () => {
    ...
    try {
      ...
      // Get the unique sample ID from the response header.
  ...
  useEffect(() => {
    ...
    mtlLoader.load(modelData.mtlUrl, (materials) => {
      ...
      objLoader.load(modelData.objUrl, (object) => {
        // Load the albedo texture using the blob URL.
  ...
  // ---------------------------
  // Use the /display endpoint to load the white model (mesh.obj) and albedo (texture.png)
  ...
  // Call the /display endpoint to load the white model and albedo.
  const handleLoadWhiteModel = async () => {
    if (!sampleFolder) {
      alert("Sample folder is not available. Please run the model first.");
      return;
    }
    try {
  ...
  const renderMaterialisedModel = () => {
    ...
    loader.load(
      ...
      (error) => {
        ...
        alert("Error loading OBJ model. Please ensure the file is valid.");
      }
    );
  ...
  // Re-render materialised model when ormImageUrl or displayData updates.
  useEffect(() => {
      ...
      <div style={{ border: '1px solid #ccc', padding: '20px' }}>
        ...
        {displayData ? (
          <div style={{ marginBottom: '10px' }}>
            <strong>White Model Loaded:</strong> {displayData.mesh_path}
          </div>
        ) : (
          <div style={{ marginBottom: '10px', color: 'red' }}>
            White model not loaded. Click "Load White Model" to retrieve it from the /display endpoint.
          </div>
        )}
        <div style={{ marginBottom: '10px' }}>
          <label>
